...
#define HOST_NAME           "ip.jsontest.com" //"<host name>"
#define HOST_PORT           80
       ...
       for (i = 0; i != noOfToken; ++i)
         {
           ...
           UART_PRINT("node: %4d, %9s, parent: %4d, children: %5d, data:\r\n%.*s, \r\n", i, type, token->parent, token->size, token->end-token->start, ptr+token->start);
       #else
           UART_PRINT("node: %4d, %9s, children: %5d, data:\r\n%.*s, \r\n", i, type, token->size, token->end-token->start, ptr+token->start);
       #endif
         }
            ...
            case 200:
            {
                        ...
                        != HTTPCli_FIELD_ID_END)
                {
                    ...
                    case 2: /* HTTPCli_FIELD_NAME_CONTENT_TYPE */
                        //{"ip": "193.190.154.184"}
                    {
                        if((!strncmp((const char *)g_buff, "application/json; charset=utf-8",
                                sizeof("application/json; charset=utf-8")))|| (!strncmp((const char *)g_buff, "application/json; charset=ISO-8859-1",
                                                                                        sizeof("application/json; charset=ISO-8859-1"))))
                        {
                            json = 1;
                            UART_PRINT("JSON DETECTED\n\r");
                        }
                        else
                        {
                            /* Note:
                ...
                if(chunked)
                {
                    ...
                    while(1)
                    {
                        ...
                        if ((bytesRead - 2) >= 0 && dataBuffer[bytesRead - 2] == '\r' && dataBuffer [bytesRead - 1] == '\n'){
                            break;
                        }
                        if(!moreFlags)
                        {
